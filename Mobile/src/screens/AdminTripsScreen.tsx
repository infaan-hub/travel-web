...
export default function AdminTripsScreen() {
  ...
  const load = () => tripsAPI.getAll().then(res => setTrips(res.data.results || res.data)).catch(console.error).finally(() => setLoading(false));
  useEffect(() => { load(); }, []);

  const openEdit = (trip?: any) => {
    if (trip) { setEditing(trip); setForm({ title: trip.title, content: trip.content, category: trip.category || '' }); }
  ...
  return (
    <View style={styles.container}>
      <View style={globalStyles.pageHeader}>
        <Text style={globalStyles.pageTitle}>Manage Trips</Text>
        <TouchableOpacity style={[globalStyles.btnPrimary, { marginTop: 12 }]} onPress={() => openEdit()}><Text style={globalStyles.btnPrimaryText}>+ Add Trip</Text></TouchableOpacity>
      </View>
      ...
      <Modal visible={modal} transparent animationType="slide">
        <ScrollView style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <View style={{ backgroundColor: '#fff', margin: 16, borderRadius: 24, padding: 24, marginTop: 60 }}>
            <Text style={{ fontSize: 20, fontWeight: '700', marginBottom: 16 }}>{editing ? 'Edit Trip' : 'Add Trip'}</Text>
